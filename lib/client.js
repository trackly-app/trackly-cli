...
async function refreshAccessToken() {
  ...
  try {
    ...
    // Backend responded but didn't return a token — treat as an invalidated session.
    // Fall through to the `clearConfig()` branch below so we don't retry a dead refresh
    // token on every subsequent 401.
  } catch (e) {
