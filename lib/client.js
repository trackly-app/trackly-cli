...
function normalizePlannedMaintenance(json, retryAfterHeader) {
  const source = json && typeof json.maintenance === 'object' && json.maintenance !== null
    ? json.maintenance
    : {};
  return {
    title: cleanText(source.title) || cleanText(json.title) || cleanText(json.error) || 'Trackly is upgrading',
    message:
    ...
    estimatedReturnPt:
      ...
      cleanText(json.estimatedReturnPt) ||
      cleanText(json.estimatedReturn) ||
      cleanText(json.estimated_return_pt) ||
      cleanText(json.estimated_return),
    retryAfterSeconds:
      cleanPositiveInt(source.retryAfterSeconds) ||
      cleanPositiveInt(source.retry_after_seconds) ||
      cleanPositiveInt(json.retryAfterSeconds) ||
      cleanPositiveInt(json.retry_after_seconds) ||
...
function isPlannedMaintenanceResponse(statusCode, json) {
  return statusCode === 503 && (
    json.code === PLANNED_MAINTENANCE_CODE ||
    json.error === PLANNED_MAINTENANCE_CODE ||
    (json.maintenance !== null && typeof json.maintenance === 'object')
  );
}
...
function refreshAccessToken() {
  ...
  // _doRefresh resolves to a token-or-null and never rejects, so the cached
  // promise can be safely shared and the finally() latch always clears.
        ...
        if (res.statusCode >= 400) {
          if (isPlannedMaintenanceResponse(res.statusCode, json)) {
            ...
            return reject({
              ...
              maintenance,
              message: formatPlannedMaintenanceMessage(maintenance),
            });
          }
