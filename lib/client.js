...
async function refreshAccessToken() {
  ...
  if (!baseUrl.startsWith('https://') && !baseUrl.startsWith('http://localhost')) {
    console.error('Warning: Refusing to send refresh token over insecure connection');
    return null;
  }
  try {
...
function apiRequest(method, endpoint, data = null, skipAuth = false, _isRetry = false) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(options, (res) => {
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode === 401 && !skipAuth && !_isRetry) {
          // Try token refresh (once only)
          const newToken = await refreshAccessToken();
          if (newToken) {
            try {
              const retried = await apiRequest(method, endpoint, data, false, true);
