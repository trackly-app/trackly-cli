...
async function refreshAccessToken() {
  ...
  try {
    ...
    // Treat as an invalidated session and purge the dead refresh token so we don't retry on
    ...
    // (Bug caught by Copilot + Cursor Bugbot + CodeRabbit on PR #20 — original code had a
    // comment promising "fall through to clearConfig()" but the clearConfig call only lived
    // in the catch block, so the 2xx-with-no-token path silently did nothing.)
    try { clearConfig(); } catch (_) {}
  } catch (e) {
