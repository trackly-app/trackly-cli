...
function contentDispositionFileName(disposition) {
  ...
  if (extended) {
    const encoded = String(extended[1] ?? extended[2] ?? '').trim();
    const separator = encoded.indexOf("''");
    const charset = separator >= 0 ? encoded.slice(0, separator).toLowerCase() : 'utf-8';
  ...
  const fileName = String(basic[1] ?? basic[2] ?? '').trim();
