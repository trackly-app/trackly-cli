...
const ANALYTICS_PREFERENCE_SYNC_TIMEOUT_MS = 1500;
...
const TRACKLY_ACCESS_GUIDANCE =
  'Trackly is in a limited rollout. Existing members can sign in; new members need a private invite. ' +
  'Request access at https://usetrackly.app/early-access.';
const TRACKLY_ACCESS_PENDING_GUIDANCE =
  'Your Trackly access request is already pending. No new request is needed.';
const TRACKLY_ACCESS_UNAVAILABLE_GUIDANCE =
  'Trackly access could not be verified right now. Please try again.';
const TRACKLY_ACCESS_CODES = new Set([
  'INVITATION_REQUIRED',
  'INVITATION_INVALID',
  'INVITATION_REDEEMED',
...
function clearConfig() {
  ...
  try {
    ...
    if (loaded && typeof loaded === 'object' && !Array.isArray(loaded)) config = loaded;
  } catch {
    // Logout must still remove an invalid or unreadable credential file. The
    // opt-out can be preserved only when the existing config is recoverable.
  }
  // A logout clears credentials and endpoint overrides, but the content-free
  // analytics opt-out must continue suppressing anonymous MCP lifecycle events.
  if (config.mcpAnalyticsOptOut === true) {
    try {
      saveConfig({ mcpAnalyticsOptOut: true });
      return;
    } catch {
      // Preserving the marker is best-effort. A disk/write failure must never
      // leave credentials behind, so fall through to unlinking the old file.
    }
  }
  const { file } = getConfigPaths();
...
function hasAuthenticationInConfig(config) {
  return Boolean(
    (process.env.TRACKLY_API_KEY || '').trim()
    || (isRecord(config) && (config.apiKey || config.token || config.refreshToken))
  );
}

function configAfterCredentialRemoval(config, scope) {
  const next = isRecord(config) ? { ...config } : {};
  if (scope === 'all' || scope === 'apiKey') delete next.apiKey;
...
async function resolveAnalyticsOptOutBeforeCredentialLoss(
  request = apiRequest,
  timeoutMs = ANALYTICS_PREFERENCE_SYNC_TIMEOUT_MS
) {
  ...
  try {
    const preference = await Promise.race([
      Promise.resolve().then(() => request(
        'GET',
        '/api/jobscout/analytics-preference',
        null,
        false,
        true,
        null,
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(() => {
          controller.abort();
          reject(new Error('analytics_preference_sync_timeout'));
        }, timeoutMs);
      }),
    ]);
    if (typeof preference?.shareUsageAnalytics !== 'boolean') {
      throw new Error('analytics_preference_invalid');
    }
    return preference.shareUsageAnalytics === false;
  } catch {
    // Fail closed: if the authoritative preference cannot be read before the
    // final credential disappears, suppress future anonymous MCP capture.
    return true;
  } finally {
...
async function synchronizeAnalyticsPreferenceBeforeLogout(
  request = apiRequest,
  options = {}
) {
  const timeoutMs = options.timeoutMs ?? ANALYTICS_PREFERENCE_SYNC_TIMEOUT_MS;
  try {
    if (!hasAuth()) return;
  } catch {
    // A corrupt or unreadable config must never prevent logout. clearConfig()
    // owns the best-effort removal path and will run immediately afterward.
    return;
  }
  const optedOut = await resolveAnalyticsOptOutBeforeCredentialLoss(request, timeoutMs);
  let config = {};
  try {
    const loaded = loadConfig();
    if (loaded && typeof loaded === 'object' && !Array.isArray(loaded)) config = loaded;
  } catch {
    // Environment-provided authentication can still make the preference
    // request valid when the on-disk config is corrupt. Replace only the
    // recoverable analytics marker and let clearConfig() remove credentials.
  }
  if (optedOut) config.mcpAnalyticsOptOut = true;
  else delete config.mcpAnalyticsOptOut;
  try {
    saveConfig(config);
  } catch {
    // Analytics synchronization is best-effort and must never block logout.
  }
}
...
async function removeCredentials({
  ...
  try {
    ...
    // Corrupt or unreadable config still belongs to the best-effort removal
    // path below. There is no safe credential state to preserve from it.
  }

  let next = configAfterCredentialRemoval(config, scope);
  const removesEphemeralCredential = scope === 'all'
    && Boolean((process.env.TRACKLY_API_KEY || '').trim());
  const removesFinalCredential = removesEphemeralCredential
    || (hasAuthenticationInConfig(config) && !hasAuthenticationInConfig(next));

  if (removesFinalCredential) {
    const optedOut = preferenceUnavailable
      ? true
      : await resolveAnalyticsOptOutBeforeCredentialLoss(request, timeoutMs);
    if (optedOut) next.mcpAnalyticsOptOut = true;
    else delete next.mcpAnalyticsOptOut;
  }

  if (scope === 'all') {
    // Preserve logout's historical behavior of clearing endpoint overrides and
    // all future config keys, with only the anonymous opt-out allowed through.
    if (next.mcpAnalyticsOptOut === true) {
      try {
        saveConfig({ mcpAnalyticsOptOut: true });
        return;
      } catch {
        // Credential removal wins over marker preservation on write failure.
      }
    }
    const { file } = getConfigPaths();
  ...
  try {
    ...
    // If the surgical rewrite cannot be persisted, do not leave known-invalid
    // credentials in the old file. Best-effort unlink is the safer fallback.
    const { file } = getConfigPaths();
    try { fs.unlinkSync(file); } catch (_) {}
    throw error;
  }
...
function createTracklyAccessError(json, status) {
  const record = nestedRecord(json);
  const code = cleanText(record.code);
  if (!code || !TRACKLY_ACCESS_CODES.has(code.toUpperCase())) return null;
  const normalizedCode = code.toUpperCase();
  const retryable = normalizedCode === 'ACCESS_CHECK_UNAVAILABLE';
  const message = retryable
    ? TRACKLY_ACCESS_UNAVAILABLE_GUIDANCE
    : normalizedCode === 'ALREADY_REQUESTED'
      ? TRACKLY_ACCESS_PENDING_GUIDANCE
      : TRACKLY_ACCESS_GUIDANCE;
  return {
    ...record,
    status: status ?? (retryable ? 503 : 403),
    code: normalizedCode,
    error: message,
    message,
    retryable,
...
async function _doRefresh() {
  ...
  try {
    ...
    // Use the centralized OAuth-only removal — NOT clearConfig() — so the user's
    // baseUrl, apiKey, and other persisted settings survive a token expiry while
    // final-credential loss still preserves the analytics preference safely.
    try { await removeCredentials({ scope: 'oauth', preferenceUnavailable: true }); } catch (_) {}
  } catch (e) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (additionalHeaders !== null) {
      const entries = (
        ...
        && Object.getPrototypeOf(additionalHeaders) === Object.prototype
      )
        ? Object.entries(additionalHeaders)
        : [];
      const idempotencyEntry = entries.find(([name]) => name.toLowerCase() === 'idempotency-key');
      const startEntry = entries.find(([name]) => name.toLowerCase() === 'x-trackly-apply-start-contract');
      const validStartHeader = startEntry
        && method === 'POST'
        && (endpoint === '/api/jobscout/apply/executions'
          || endpoint === '/api/jobscout/apply/executions/recover')
        && startEntry[1] === APPLY_CONTRACT.contractVersion;
      if (
        entries.length !== (startEntry ? 2 : 1)
        || (startEntry && !validStartHeader)
        || !idempotencyEntry
        || typeof idempotencyEntry[1] !== 'string'
        || !/^[\x20-\x7e]{16,200}$/.test(idempotencyEntry[1])
      ) {
        reject(new Error('Only a valid Idempotency-Key additional header is permitted.'));
        return;
      }
      headers['Idempotency-Key'] = idempotencyEntry[1];
      if (startEntry) headers['X-Trackly-Apply-Start-Contract'] = startEntry[1];
    }
    const authMethod = getAuthMethod();
    const requestedTimeoutMs = Number(requestOptions?.timeoutMs);
    const timeoutMs = Number.isFinite(requestedTimeoutMs) && requestedTimeoutMs > 0
      ? Math.min(getRequestTimeoutMs(), requestedTimeoutMs)
      : getRequestTimeoutMs();
    const signal = requestOptions?.signal;
    if (signal?.aborted) {
      reject(new Error('Trackly request aborted.'));
      return;
    }
    ...
    const req = lib.request(options, (res) => {
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode >= 400) {
          ...
          const accessError = createTracklyAccessError(json, res.statusCode);
    ...
    const abortRequest = () => req.destroy(new Error('Trackly request aborted.'));
    ...
    const req = lib.request({
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode >= 400) {
          ...
          const accessError = createTracklyAccessError(body, res.statusCode);
          if (accessError) {
            reject(accessError);
            return;
          }
        ...
        resolve({
          ...
          fileName: contentDispositionFileName(disposition),
          sha256: res.headers['x-trackly-file-sha256'] || null,
          applyRunId: res.headers['x-trackly-apply-run-id'] || null,
          resumeId: res.headers['x-trackly-resume-id'] || null,
        });
      });
    });
...
function contentDispositionFileName(disposition) {
  ...
  if (extended) {
    const encoded = extended[1] != null ? extended[1] : String(extended[2] ?? '').trim();
    const match = encoded.match(/^([^']*)'[^']*'(.*)$/s);
    const charset = match ? match[1].toLowerCase() : 'utf-8';
    const payload = match ? match[2] : encoded;
    if (charset === 'utf-8' || charset === 'utf8') {
      try { return decodeURIComponent(payload); } catch (_) {}
    }
  }
  const basic = value.match(/(?:^|;)\s*filename\s*=\s*(?:"([^"]*)"|([^;]*))/i);
  if (!basic) return null;
  const fileName = basic[1] != null ? basic[1] : String(basic[2] ?? '').trim();
...
module.exports = {
  ...
  removeCredentials,
  saveConfig,
  synchronizeAnalyticsPreferenceBeforeLogout,
