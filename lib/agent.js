...
function canonicalSkillDir() {
  return path.join(getConfigPaths().dir, 'skills', SKILL_NAME);
}
...
function isManagedDirectory(target) {
  try {
    ...
    return metadata.managedBy === 'trackly-cli' && metadata.skill === SKILL_NAME;
  } catch (_) {
    return false;
  }
}
...
function linkClientSkill(client, canonical) {
  ...
  try {
    ...
    if (stat.isSymbolicLink() || isManagedDirectory(target)) fs.rmSync(target, { recursive: true, force: true });
    else throw new Error(`Refusing to overwrite unmanaged skill at ${target}`);
  } catch (error) {
  ...
  try {
    ...
    fs.writeFileSync(path.join(target, MANAGED_FILE), managedMetadata({ copiedFrom: canonical }), { mode: 0o600 });
...
function isMcpRegistered(client) {
  ...
  return client === 'codex'
    ? fileContains(path.join(process.env.CODEX_HOME || path.join(os.homedir(), '.codex'), 'config.toml'), /mcp_servers\.trackly|\[mcp_servers\.trackly\]/)
    : claudeConfigFiles.some((file) => fileContains(file, /"trackly"\s*:/));
}

function registerMcp(client) {
  if (isMcpRegistered(client)) return { status: 'current' };
...
function inspectClient(client) {
  ...
  const mcpRegistered = isMcpRegistered(client);
  return { client, executable, installed, mcpRegistered, target };
}
...
function validateResumeFile(download) {
  ...
  const isPdf = download.contentType === 'application/pdf' && download.buffer.subarray(0, 5).toString() === '%PDF-';
  const isDocx = download.contentType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    && download.buffer[0] === 0x50 && download.buffer[1] === 0x4b;
  if (!isPdf && !isDocx) throw new Error(`Unsupported or invalid resume type: ${download.contentType}`);
  return { digest, contentType: download.contentType };
}

async function prepareResume() {
  if (!hasAuth()) throw new Error('Authenticate first with `trackly login` or `trackly config --api-key ...`.');
  cleanResumeCache();
  const download = await downloadFile('/api/jobscout/application-profile/default-resume', `trackly-mcp/${SKILL_VERSION}`);
  try {
    const validated = validateResumeFile(download);
    const dir = cacheDir();
    ensureDir(dir, 0o700);
    const name = `${Date.now()}-${safeResumeName(download.fileName, download.contentType)}`;
