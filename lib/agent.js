...
const MIN_APPLY_PROTOCOL_VERSION = '3.1.0';
...
function protocolAtLeast(version, minimum = MIN_APPLY_PROTOCOL_VERSION) {
  ...
  if (actual.length < 2 || actual.some((part) => !Number.isInteger(part) || part < 0)) return false;
...
async function doctorAgent() {
  ...
  try {
    ...
    report.compatible = Number(report.protocol.compatibleSkillMajor) === SKILL_MAJOR
      && protocolAtLeast(report.protocol.version, MIN_APPLY_PROTOCOL_VERSION);
