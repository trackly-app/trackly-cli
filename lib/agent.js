...
function linkClientSkill(client, canonical) {
  ...
  try {
    ...
    if (!stat.isSymbolicLink() && !isManagedDirectory(target)) {
...
function isMcpRegistered(client) {
  ...
  return client === 'codex'
    ? fileContains(path.join(process.env.CODEX_HOME || path.join(os.homedir(), '.codex'), 'config.toml'), /^\s*\[mcp_servers\.trackly\]\s*(?:#.*)?$/m)
...
function inspectClient(client) {
  ...
  return { client, executable, installed, installedSkillVersion: metadata?.skillVersion || null, mcpRegistered, target };
}
...
function validateResumeFile(download) {
  ...
  const isDocx = download.buffer[0] === 0x50 && download.buffer[1] === 0x4b;
