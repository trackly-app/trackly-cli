...
function cleanResumeCache(now = Date.now()) {
  ...
  function cleanDirectory(current) {
...
function resumeConfirmation(fileName, sha256, sizeBytes, exactLocalPath) {
  return {
    required: true,
...
async function prepareResume(userAgent = MCP_USER_AGENT) {
  ...
  try {
    ...
    const dir = cacheDir();
    ensureDir(dir, 0o700);
    const name = resumeCacheName(download.fileName, validated.contentType);
    ...
    return {
      ...
      confirmation: resumeConfirmation(fileName, validated.digest, download.buffer.length, destination),
    };
  } finally {
...
async function doctorAgent() {
  ...
  try {
    ...
    if (report.profile.hasDefaultResume) {
      try {
        const prepared = await prepareResume(CLI_USER_AGENT);
