...
OAuth authorization metadata must pass the bundled MCP SDK OAuthMetadataSchema, including optional members, before discovery is considered usable. Every positive reviewer fixture must retain a non-empty string array of forbidden actions (`mustNot`); a prompt and expected result alone do not preserve the safety evidence.
