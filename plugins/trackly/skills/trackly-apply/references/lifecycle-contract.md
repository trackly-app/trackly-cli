...
## Approve and attach the exact resume

When the visible form has a resume control, call `trackly_prepare_resume_artifact` with `operation: preview`. Its optional `document` is the backend-owned original and includes `resumeId`, filename, MIME type, SHA-256, size, expiry, but no model-visible capability URL. The preview component receives private access through component-only metadata. Ask the user to inspect the original there; never ask them to paste its private URL into chat. Keep it out of logs, progress payloads, employer fields, and handoffs. A preview proves neither user approval nor employer attachment.

After the user explicitly approves that exact document, call `trackly_report_apply_progress` with `operation: approve_resume`. Supply the current `executionId`, `expectedRevision`, execution `originalSnapshotHash`, current `profileRevision`, the unchanged preview `resumeId`, `resumeSha256`, `resumeFilename`, and `resumeSizeBytes`, the current execution's expiresAt converted to the schema's date-time form, literal `explicitUserResumeApproval: true`, and a fresh idempotency key. The preview capability has its own shorter expiry and does not define the approval lifetime. Continue only after the exact approval succeeds. A changed document, profile revision, execution snapshot, authorization grant, or expiry requires a fresh preview and approval.

After approval, try the [approved download transfer](approved-download-transfer.md). Approval does not make host upload available. Automatic attachment is allowed only when the active host provides a documented secure host-mediated primitive that can materialize the original file without exposing the component-only capability to model context and select it in the visible editable employer file input, and the agent verifies the materialized SHA-256 and size against the approval immediately before attachment. Opening a URL or embedded resource is not a file-import guarantee for OpenAI or any other host. If those capabilities or checks are absent, ask the user to download through the preview component and upload manually and keep that browser-local upload unbound. Use replacement semantics and one attachment action; never append, replay, or select a second file over an already committed input.

After a supported attachment, verify the visible committed filename and that the control still represents the same materialized file. Record `operation: record_observations` with an observation whose `observationType` is `resume_attachment`, whose `resolutionCode` is `attached`, and whose current run/batch/member/inspection binding and metadata include `scenarioCode: resume_upload`, `committed: true`, current `browserSurface` and `browserBindingHash`, exact approved `resumeId`, `resumeSha256`, `resumeSizeBytes`, and the lowercase SHA-256 of the exact approved filename as `resumeFilenameSha256`. The provider and field label stay redacted and stable. If attachment is removed or fails, record `removed` or `failed` with the same identity so it becomes the latest observation. Never record `attached` for a manual upload, from a visible filename alone, or from stale browser evidence.
...
- `resumeDependency: approved` plus exact `resumeId`, `resumeSha256`, and current `browserBindingHash` only when the same execution has current exact approval and the latest current-epoch browser observation proves the committed attachment
- literal `resumeDependency: not_applicable` with all resume identity fields omitted when no resume was required or the user performed an unbound manual upload

The backend reads the current membership hash, profile revision, run set, expiry, private lease, approval, original stored bytes, and latest current-browser attachment evidence inside the same transaction, then atomically persists the review checkpoint, truth certification, and `review_ready` outcome. A manually uploaded resume is browser-local, unbound, and not attested by trackly; its visible filename check is part of the user handoff, not the certification. Send resume identity only in the `approved` branch; never send filenames, paths, contents, capability URLs, answer values, or server-owned internals in certification. Do not report success unless the response proves the durable review-ready state.
